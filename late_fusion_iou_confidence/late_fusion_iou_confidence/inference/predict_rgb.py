...
def decode_prediction(output, image_size, conf_thresh=0.3):
    ...
    pred_box = output[1:, :, :].mean(dim=[1, 2]).tolist()  # [x, y, w, h]
    ...
    return {
        "bbox": [x1, y1, x2, y2],
        "score": pred_cls,
        "label": 0  # one class only
    }

def main():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = load_model(model_type='rgb', weights_path='results/logs/rgb/epoch_50.pth', device=device)
    model.eval()

    test_folder = "data/images_rgb"
    output_json = "results/fused_detections/preds_rgb.json"
    ...
    print(f"Saved RGB predictions to {output_json}")
